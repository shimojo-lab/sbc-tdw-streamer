...
const bool ConfigParser::readParams(const _pt::ptree& conf){
    try{
        ...
        this->fb_dev = conf.get_optional<std::string>("display.framebuffer_device").get();
        this->rbuf_size = conf.get_optional<int>("display.recv_buffer_size").get();
        this->vbuf_size = conf.get_optional<int>("display.view_buffer_size").get();
    }catch(...){
...
const dc_params_t ConfigParser::getDisplayClientParams(){
    ...
    return std::forward_as_tuple(ip, port, fb_dev, rbuf_size, vbuf_size);
}
