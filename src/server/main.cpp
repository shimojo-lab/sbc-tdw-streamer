...
int main(int argc, char *argv[]){
    ...
    for(int i=0; i<display_num; ++i){
        thread_list[i] = boost::thread(thread_task, ios, splitter.getFrameQueuePtr(i), ip_list[i].c_str(), port_list[i]);
    }
    
    // フレーム分割を開始
    splitter.start();
    c_lock(), std::cout << "[Info] Ended play video." << std::endl;
...
/* 別スレッド実行用の関数 */
void thread_task(const smt_ios_t ios, const smt_fq_t queue, const char* const ip, const int port){
    // 分割フレーム送信器を起動
